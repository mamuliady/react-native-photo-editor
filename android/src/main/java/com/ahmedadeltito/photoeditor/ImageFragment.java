    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (stickers != null && stickers.size() > 0) {
            ...
            for (int i = 0;i < stickers.size();i++) {
                stickerBitmaps.add(decodeSampledBitmapFromResource(getActivity().getResources(), stickers.get(i), 600, 600));
            }
        } else {
            stickerBitmaps = new ArrayList<>();
            for (int i = 0; i < images.length(); i++) {
                stickerBitmaps.add(decodeSampledBitmapFromResource(photoEditorActivity.getResources(), images.getResourceId(i, -1), 600, 600));
            }
        }
    }
